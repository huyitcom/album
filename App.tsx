...
import {
  spreadsData as initialSpreadsData,
  libraryImages as initialLibraryImages,
} from "./constants";
...
const App: React.FC = () => {
  ...
  const [libraryImages, setLibraryImages] =
    useState<AlbumImage[]>(initialLibraryImages);
  ...
  const [isRandomDesignPickerOpen, setIsRandomDesignPickerOpen] =
    useState(false);
  ...
  const [textPickerTargetSpreadId, setTextPickerTargetSpreadId] = useState<
    string | null
  >(null);
  const [isStickerPickerOpen, setIsStickerPickerOpen] = useState(false);
  const [stickerPickerTargetSpreadId, setStickerPickerTargetSpreadId] =
    useState<string | null>(null);
  const [isMobileDesignPickerOpen, setIsMobileDesignPickerOpen] =
    useState(false);
  ...
  const [editHintTarget, setEditHintTarget] = useState<{
    spreadId: string;
    slotId: string;
  } | null>(null);
  ...
  const [addOverlayTargetSpreadId, setAddOverlayTargetSpreadId] = useState<
    string | null
  >(null);
  ...
  const [touchDragOverSlot, setTouchDragOverSlot] = useState<{
    spreadId: string;
    slotId: string;
  } | null>(null);
  ...
  const handleAppClick = (e: MouseEvent) => {
    ...
    if (
      !target.closest('[data-text-element="true"]') &&
      !target.closest('[data-text-toolbar="true"]') &&
      !target.closest('[data-sticker-element="true"]')
    ) {
  ...
  const handleChangeLayout = (spreadId: string, layoutId: string) => {
    setSpreads((currentSpreads) => {
      ...
      const imagesToReturn = Object.values(oldSpread.images).map(
        (placed: PlacedImageData) => placed.image,
      );
      ...
      setLibraryImages((currentImages) => {
        ...
        return currentImages.map((libImg) => {
          if (usageToDecrement.has(libImg.id)) {
            return {
              ...libImg,
              used: libImg.used - (usageToDecrement.get(libImg.id) || 0),
            };
          }
          return libImg;
        });
  ...
  const handleDropImageInSlot = useCallback(
    (imageId: string, spreadId: string, slotId: string) => {
  ...
  const handleSwapImagesInSlots = (
    ...
    if (
      source.spreadId === target.spreadId &&
    ...
    setSpreads((currentSpreads) => {
      const sourceSpreadIndex = currentSpreads.findIndex(
        (s) => s.id === source.spreadId,
      );
      const targetSpreadIndex = currentSpreads.findIndex(
        (s) => s.id === target.spreadId,
      );
      ...
      if (sourceSpreadIndex === targetSpreadIndex) {
        ...
        // Different spreads
        const newSourceSpread = {
          ...sourceSpread,
          images: { ...sourceSpread.images },
        };
        const newTargetSpread = {
          ...targetSpread,
          images: { ...targetSpread.images },
        };
  ...
  const handleUpdateImageTransform = (
    spreadId: string,
    slotId: string,
  ...
  const handleUpdateTotalPages = (newTotalPages: number) => {
    const targetSpreads = Math.max(1, Math.round(newTotalPages / 2));
    if (targetSpreads < spreads.length) {
      // Check if there are any contents on the pages that would be removed
      const spreadsToRemove = spreads.slice(targetSpreads);
      const hasContent = spreadsToRemove.some(
        (spread) =>
          Object.keys(spread.images).length > 0 ||
          spread.texts.length > 0 ||
          spread.stickers.length > 0,
      );

      if (hasContent) {
        const confirmMsg =
          language === "vn"
            ? "Các trang sắp xóa đang chứa hình ảnh, chữ hoặc nhãn dán. Bạn có chắc chắn muốn giảm số trang và xóa các nội dung này không?"
            : "The pages you are removing contain photos, text, or stickers. Are you sure you want to reduce the page count and delete this content?";
      ...
      // Slicing and updating library usage for removed images
      setSpreads((currentSpreads) => {
        const slashedSpreads = currentSpreads.slice(0, targetSpreads);
        const removedSpreads = currentSpreads.slice(targetSpreads);

        // Decrement usage counts for removed images
        setLibraryImages((currentImages) => {
          const usageToDecrement = new Map<string, number>();
          removedSpreads.forEach((spread) => {
            Object.values(spread.images).forEach((placedImage: any) => {
              const imgId = placedImage.image.id;
              usageToDecrement.set(
                imgId,
                (usageToDecrement.get(imgId) || 0) + 1,
              );
            });
          });

          if (usageToDecrement.size === 0) return currentImages;

          return currentImages.map((img) => {
            if (usageToDecrement.has(img.id)) {
              return {
                ...img,
                used: Math.max(
                  0,
                  (img.used || 0) - usageToDecrement.get(img.id)!,
                ),
              };
            }
            return img;
        ...
        return slashedSpreads;
      });
    } else if (targetSpreads > spreads.length) {
      // Add empty spreads
      setSpreads((currentSpreads) => {
        const newSpreads = [...currentSpreads];
        const currentLen = newSpreads.length;
        const needed = targetSpreads - currentLen;
        for (let i = 0; i < needed; i++) {
          const lastSpread = newSpreads[newSpreads.length - 1];
          const newPageStart = lastSpread ? lastSpread.pages[1] + 1 : 1;
          newSpreads.push({
    ...
    newSpreads.forEach((spread) => {
      Object.values(spread.images).forEach((placedImage: PlacedImageData) => {
        usageCounts.set(
          ...
          (usageCounts.get(placedImage.image.id) || 0) + 1,
        );
      });
    });
  ...
  const handleConfirmRandomDesign = (selectedLayoutIds: string[]) => {
    ...
    const tempSpreads = spreads.map((spread) => {
      const randomLayoutId =
        selectedLayoutIds[Math.floor(Math.random() * selectedLayoutIds.length)];
      return {
        ...spread,
  ...
  const handleRemoveImageFromLibrary = (imageIdToRemove: string) => {
    ...
    const newSpreads = spreads.map((spread) => {
      ...
      Object.entries(newImages).forEach(
        ([slotId, placedImage]: [string, PlacedImageData]) => {
  ...
  const handleAddTextFromTemplate = (template: {
    style: TextStyle;
    ...
    setSpreads((currentSpreads) =>
      currentSpreads.map((spread) =>
        spread.id === textPickerTargetSpreadId
          ? { ...spread, texts: [...spread.texts, newText] }
          : spread,
      ),
    );
  ...
  const handleUpdateText = (
    spreadId: string,
    textId: string,
    newTextData: Partial<TextElement>,
  ) => {
    setSpreads((currentSpreads) =>
      currentSpreads.map((spread) => {
        if (spread.id === spreadId) {
    ...
    setSpreads((currentSpreads) =>
      currentSpreads.map((spread) =>
        spread.id === spreadId
          ? {
              ...spread,
              texts: spread.texts.filter((text) => text.id !== textId),
            }
          : spread,
      ),
    );
  ...
  const handleAddSticker = (stickerUrl: string) => {
    ...
    setSpreads((currentSpreads) =>
      currentSpreads.map((spread) =>
        spread.id === stickerPickerTargetSpreadId
          ? { ...spread, stickers: [...spread.stickers, newSticker] }
          : spread,
      ),
    );
  ...
  const handleUpdateSticker = (
    spreadId: string,
    stickerId: string,
    newStickerData: Partial<StickerElement>,
  ) => {
    setSpreads((currentSpreads) =>
      currentSpreads.map((spread) => {
        if (spread.id === spreadId) {
  ...
  const handleSaveProject = async (projectName: string) => {
    ...
    try {
      ...
      const cleanedLibraryImages: SavedAlbumImage[] = libraryImages.map(
        ({ url, ...rest }) => rest,
      );
      ...
      const cleanedSpreads: SavedSpreadData[] = spreads.map((spread) => ({
        ...
        images: Object.fromEntries(
          ...
          Object.entries(spread.images).map(
            ([slotId, placedImage]: [string, PlacedImageData]) => {
      ...
      localStorage.setItem(
        `photobook_project_${projectName}`,
      ...
      alert(
        "An unexpected error occurred while saving the project. Your browser's storage might be full or IndexedDB is not supported.",
      );

      // Clean up any partially saved image data on failure
      for (const img of libraryImages) {
        await deleteImageFromDB(img.id).catch((e) =>
          console.error(`Cleanup failed for image ${img.id}`, e),
        );
      }
      return false;
    }
  ...
  const handleLoadProject = async (projectName: string) => {
    ...
    if (savedData) {
      try {
        ...
        const newLibraryImagesPromises = projectData.libraryImages.map(
          async (img: SavedAlbumImage) => {
        ...
        const newLibraryImages = (
          await Promise.all(newLibraryImagesPromises)
        ...
        const newSpreads: SpreadData[] = projectData.spreads.map(
          (spread: SavedSpreadData) => {
        ...
        alert(
          "Failed to load project. The project file might be corrupted or image data is missing.",
        );
      }
    }
  };
  ...
  const handleDeleteProject = async (projectName: string) => {
    ...
    if (savedData) {
      try {
        ...
        if (
          projectData.libraryImages &&
          Array.isArray(projectData.libraryImages)
        ) {
          for (const img of projectData.libraryImages) {
            await deleteImageFromDB(img.id).catch((e) =>
              console.error(`Failed to delete image ${img.id} from DB`, e),
            );
          }
        }
      } catch (e) {
        console.error(
          `Could not parse project ${projectName} for deletion, image files may be orphaned.`,
        );
      }
    }
  ...
  const handleImageTouchStart = (
    image: AlbumImage,
    startEvent: React.TouchEvent<HTMLDivElement>,
  ) => {
    if (!isMobile) return;
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (dropTarget) {
        ...
        if (slotElement && spreadElement) {
          ...
          if (slotId && spreadId) {
            if (
              touchDragOverSlot?.spreadId !== spreadId ||
              touchDragOverSlot?.slotId !== slotId
            ) {
    ...
    const handleTouchEnd = () => {
      if (touchDragItem && touchDragOverSlot) {
        handleDropImageInSlot(
          touchDragItem.image.id,
          touchDragOverSlot.spreadId,
          touchDragOverSlot.slotId,
        );
      }
  ...
  useEffect(() => {
    const originalElementId = touchDragItem
      ? `lib-img-thumb-${touchDragItem.image.id}`
      : null;
  ...
  return (
    <div className="flex flex-col h-full font-sans bg-gray-200 text-gray-800">
      ...
      <input
        type="file"
        accept="image/*"
        ref={addOverlayImageFileInputRef}
      ...
      {showMobileDragHint && (
        <MobileDragHint
          ...
          onClose={() => setShowMobileDragHint(false)}
        />
      )}
