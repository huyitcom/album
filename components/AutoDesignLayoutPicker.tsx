...
interface AutoDesignLayoutPickerProps {
  isMobile: boolean;
  onSelectLayout: (layoutId: string) => void;
  onClose: () => void;
}

const AutoDesignLayoutPicker: React.FC<AutoDesignLayoutPickerProps> = ({ isMobile, onSelectLayout, onClose }) => {
    ...
    >
      ...
      >
        <header className="flex items-center justify-between p-4 border-b">
          <div>
            <h2 className="text-lg font-semibold">{t('designForMeTitle')}</h2>
            <p className="text-sm text-gray-500">{t('designForMeSubtitle')}</p>
          </div>
...
export default AutoDesignLayoutPicker;