    ...
    <header className="flex items-stretch shadow-sm border-b border-gray-300">
      ...
      <div className="bg-cyan-500 flex-shrink-0 flex items-center space-x-2 px-2 md:px-4 py-2">
        ...
        <div className="relative md:hidden" ref={mobileMenuRef}>
          ...
          {isMobileMenuOpen && (
            <div
              className="absolute top-full left-0 mt-2 w-64 bg-gray-700 rounded-md shadow-lg z-50 border border-gray-600 text-gray-200"
              role="menu"
            >
              <div className="p-2 space-y-1">
                ...
                <div className="px-3 pt-2 pb-1 text-xs text-gray-400 font-semibold uppercase">
                  {t("settings")}
                </div>
                ...
                {/* Page Count Selection Mobile */}
                <div className="px-3 py-1 pb-3">
                  <span className="text-xs text-gray-500 block mb-1">
                    {language === "vn" ? "Số trang" : "Number of Pages"}
                  </span>
                  <div className="flex items-center space-x-2">
                    <button
                      type="button"
                      onClick={() => onUpdateTotalPages(totalPages - 2)}
                      className="flex items-center justify-center w-8 h-8 rounded bg-gray-600 text-white font-semibold cursor-pointer text-lg hover:bg-gray-550 active:bg-gray-500 select-none"
                    >
                      -
                    </button>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!isNaN(val)) {
                          onUpdateTotalPages(val);
                        }
                      }}
                      className="w-16 h-8 text-center border border-gray-600 rounded text-sm text-gray-200 bg-gray-650 focus:outline-none focus:border-blue-500"
                    />
                    <button
                      type="button"
                      onClick={() => onUpdateTotalPages(totalPages + 2)}
          ...
          <div className="relative hidden md:block" ref={settingsRef}>
            ...
            {isSettingsOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-md shadow-lg z-50 border border-gray-200 p-4">
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 border-b pb-2">
                  {t("settings")}
                </h3>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      type="button"
                      onClick={() => onUpdateTotalPages(totalPages - 2)}
                      className="flex items-center justify-center w-8 h-8 rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-400 font-semibold cursor-pointer text-lg select-none"
                    >
                      -
                    </button>
                    <input
                      ...
                      className="flex-1 text-center h-8 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-800 bg-white"
                    />
                    <button
                      type="button"
                      onClick={() => onUpdateTotalPages(totalPages + 2)}
